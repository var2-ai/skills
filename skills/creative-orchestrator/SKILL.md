...
- `veo-3.1` (default for non-cinematic) — strong all-around, Hebrew/Arabic narration in scene, native audio + voice-over support, `aspect_ratio: portrait | landscape`. Use when non-Latin language matters more than camera language.
- `ltx-2.3` — cinematic *with native audio* + per-second pricing. Good middle ground when you need Hebrew/Arabic voice-over and a cinematic feel, but seedance still wins on pure motion quality (and also has native audio — they're peers on audio, not opposites).
